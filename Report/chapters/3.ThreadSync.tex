...
Il Costrutto \textit{Synchronized} si dice Rientrante in quanto qualora un Thread eseguente un blocco di codice sincronizzato volesse eseguirne un altro controllato tramite lo stesso Monitor, gli verrà concessa tale opportunità.
\subsection{Esempio di Sincronizzazione tra due Thread tramite il Costrutto \texttt{synchronized} Rientrante}
Si riporta a seguire una reimplementazione del Precedente Programma e della sua relativa esecuzione al fine di illustrare le proprietà di Sincronizzazione e di Rientranza del Costrutto descritto. In quanto la classe contenente il metodo \texttt{main()} e quella per la generazione dei \texttt{Thread} sono analoghe e non hanno subito alcuna modifica, si riporta solo la classe rappresentante l'oggetto contatore.
\input{images/SynchronizedReentranceExample.tex}
\subsection{Considerazioni}
A fronte di una semplice implementazione, l'utilizzo del Costrutto \texttt{synchronized} porta prestazioni non desiderabili in quanto un solo \textit{Thread} alla volta può accedere ad un Blocco di Codice controllato dallo stesso Oggetto. Altro svantaggio è ben visibile nell'esempio riportato poc'anzi ed è quello di non avere una garanzia sulla sequenza di esecuzione dei vari \textit{Thread}. È dunque possibile incorrere in fenomeni di \textit{Starvation}, il Costrutto infatti non è Equo.
...
\subsection{Esempio di Sincronizzazione tra due \textit{Thread} tramite \texttt{Semaphore}}. Nuovamente si riporta solamente la reimplementazione del Contatore in quanto non sono state apportate ulteriori modifiche al codice.
\input{images/SemaphoreSynchronization.tex}
\subsection{Considerazioni}
L'utilizzo dei Semafori, in particolare nel caso di Semafori Generalizzati, in un Programma permette di aumentare il parallelismo di un'applicazione in cui le risorse condivise possono essere molteplici di simile natura come ad esempio nel problema dei Produttori e dei Consumatori. A tale vantaggio bisogna tuttavia affiancare come fatto in precedenza per l'utilizzo dell'Interfaccia \texttt{Lock} una maggiore complessità nell'utilizzo rispetto al Costrutto \texttt{synchronized} che rende il Semaforo più prono ad errori logici.